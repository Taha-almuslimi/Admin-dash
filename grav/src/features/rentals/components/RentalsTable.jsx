          ...
          <tbody className="divide-y divide-brand-border">
            {rentals.map((rental) => (
              <tr key={rental.id} className="hover:bg-brand-content/50 transition-colors cursor-pointer" onClick={() => onOpenDrawer(rental)}>
                ...
                <td className="px-6 py-4 text-center">
                  <button onClick={(e) => { e.stopPropagation(); onOpenDrawer(rental); }} className="p-1.5 text-brand-info hover:bg-brand-info/10 rounded-lg transition-colors" title="عرض العقد">
                    <FileText size={18} />
                  </button>
                </td>
              </tr>
