...
export default function ComplaintsTable({ setShowModal }) {
  return (
            ...
            {complaintsData.map(complaint => (
              <tr key={complaint.id} className="hover:bg-brand-content/50 transition-colors">
                ...
                <td className="px-6 py-4">
                  <span className="px-2.5 py-1 bg-brand-info/10 text-brand-info rounded-md text-xs font-bold">{complaint.type}</span>
                </td>
                ...
                <td className="px-6 py-4 text-center">
                  {complaint.priority === 'high' ? <span className="text-brand-danger font-bold text-xs">🔴 عالية</span> : <span className="text-brand-text-muted text-xs">عادية</span>}
                </td>
                <td className="px-6 py-4 text-center">
                  <span className="px-2.5 py-1 bg-brand-warning/10 text-brand-warning rounded-md text-xs font-bold">{complaint.status}</span>
                </td>
                <td className="px-6 py-4 text-center">
