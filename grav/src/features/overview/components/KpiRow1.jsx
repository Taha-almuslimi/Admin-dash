...
export default function KpiRow1({
  stats = { rentals: '0', profits: '0', disputes: '0', users: '0' },
  loading = false,
}) {
  return (
    <div className="grid grid-cols-1 md:grid-cols-2 xl:grid-cols-4 gap-6">
      <div className="bg-brand-card rounded-xl p-6 border border-brand-border shadow-sm flex flex-col justify-between hover:shadow-md transition-shadow">
        ...
        <div>
          ...
          {loading ? <Skeleton className="h-8 w-24" /> : <p className="text-[32px] font-bold text-brand-text-primary leading-none">{stats?.rentals}</p>}
        </div>
      </div>
      ...
      <div className="bg-brand-card rounded-xl p-6 border border-brand-border shadow-sm flex flex-col justify-between hover:shadow-md transition-shadow">
        ...
        <div>
          ...
          {loading ? <Skeleton className="h-8 w-32" /> : (
            <p className="text-[32px] font-bold text-brand-text-primary leading-none">{stats?.profits} <span className="text-base font-normal text-brand-text-muted">ر.ي</span></p>
          )}
        </div>
      </div>
      ...
      <div className="bg-brand-card rounded-xl p-6 border border-brand-border shadow-sm flex flex-col justify-between hover:shadow-md transition-shadow">
        ...
        <div>
          ...
          {loading ? <Skeleton className="h-8 w-16" /> : <p className="text-[32px] font-bold text-brand-text-primary leading-none">{stats?.disputes}</p>}
        </div>
      </div>
      ...
      <div className="bg-brand-card rounded-xl p-6 border border-brand-border shadow-sm flex flex-col justify-between hover:shadow-md transition-shadow">
        ...
        <div>
          ...
          {loading ? <Skeleton className="h-8 w-24" /> : <p className="text-[32px] font-bold text-brand-text-primary leading-none">{stats?.users}</p>}
        </div>
      </div>
    </div>
  );
